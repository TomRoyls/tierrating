...
public class TraktTokenRefresher implements TokenRefresher {
	...
	public void refresh(User user) {
		MediaSourceConnection connection = user.getConnections().get(MediaSource.TRAKT);
		if (connection.getExpiresOn().isBefore(LocalDateTime.now().plusDays(2))) {
			try {
				...
				if (!response.isSuccessful())
					throw new ThirdPartyAuthenticationException("Refreshing trakt access token failed");
				...
				throw new ThirdPartyAuthenticationException("Failed to refresh Trakt token: " + e.getMessage());
			}
		}
	}
}
